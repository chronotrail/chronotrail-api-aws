...
def create_error_response(
    ...
    # Handle file processing errors
        ...
        error_response = ErrorResponse(
            ...
            details=(
                {"file_type": error.file_type, "processing_type": error.processing_type}
                if include_details
                else None
            ),
        )
...
async def chronotrail_exception_handler(
    request: Request, exc: ChronoTrailError
) -> JSONResponse:
    """
...
async def aws_service_exception_handler(
    ...
    if isinstance(
        exc, (S3Error, TextractError, TranscribeError, RekognitionError, BedrockError)
    ):
        if (
            "access denied" in exc.message.lower()
            or "unauthorized" in exc.message.lower()
        ):
            status_code = status.HTTP_403_FORBIDDEN
