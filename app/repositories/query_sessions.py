...
class QuerySessionRepository(BaseRepository[DBQuerySession, QuerySessionCreate, QuerySessionUpdate]):
    """
    ...
    async def get_active_sessions_by_user(
        ...
        ).order_by(DBQuerySession.updated_at.desc()).offset(skip).limit(limit)
    ...
    async def cleanup_old_sessions_by_user(
        ...
        keep_stmt = select(DBQuerySession.id).where(
            DBQuerySession.user_id == user_id
        ).order_by(DBQuerySession.updated_at.desc()).limit(keep_count)
        ...
        count_stmt = select(func.count(DBQuerySession.id)).where(
            and_(
                DBQuerySession.user_id == user_id,
                ~DBQuerySession.id.in_(keep_ids)
            )
        )
        result = await db.execute(count_stmt)
    ...
    async def get_session_statistics(
        ...
        duration_stmt = select(
            func.avg(
                func.extract('epoch', DBQuerySession.updated_at - DBQuerySession.created_at)
            )
        ).where(base_filter)
        ...
        return {
            ...
            "average_duration_minutes": round(avg_duration_seconds / 60, 2) if avg_duration_seconds else 0
        }
            ...
            if filters["has_media_references"]:
                stmt = stmt.where(
                    and_(
                        ...
                        DBQuerySession.session_context.op('->>')('media_references').isnot(None)
                    )
                )
            else:
                stmt = stmt.where(
                    or_(
                        ...
                        DBQuerySession.session_context.op('->>')('media_references').is_(None)
                    )
                )
        ...
        if "last_query_contains" in filters:
            stmt = stmt.where(
                and_(
                    ...
                    DBQuerySession.last_query.ilike(f"%{filters['last_query_contains']}%")
                )
            )
