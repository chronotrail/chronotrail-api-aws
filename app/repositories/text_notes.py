...
class TextNoteRepository(BaseRepository[DBTextNote, TextNoteCreate, None]):
    ...
    async def get_by_user(
        ...
        stmt = select(DBTextNote).where(
            and_(DBTextNote.id == note_id, DBTextNote.user_id == user_id)
        )
        result = await db.execute(stmt)
        return result.scalars().first()
        ...
        stmt = (
            ...
            .where(
                and_(
                    ...
                    DBTextNote.latitude.between(
                        latitude - lat_delta, latitude + lat_delta
                    ),
                    DBTextNote.longitude.between(
                        longitude - lng_delta, longitude + lng_delta
                    ),
                )
            )
            .order_by(DBTextNote.timestamp.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def search_by_names(
        ...
        if search_terms:
            ...
            for term in lower_terms:
                conditions.append(
                    func.lower(func.array_to_string(DBTextNote.names, " ")).contains(
                        term
                    )
                )
    ...
    async def update(
        self, db: AsyncSession, *, db_obj: DBTextNote, obj_in: None
    ) -> DBTextNote:
        """
    ...
    async def get_recent_by_user(
        ...
        stmt = (
            select(DBTextNote)
            .where(DBTextNote.user_id == user_id)
            .order_by(DBTextNote.timestamp.desc())
            .limit(limit)
        )
    ...
    def _apply_filters(self, stmt, filters: Dict[str, Any]):
        ...
        if "has_location" in filters:
            if filters["has_location"]:
                ...
                stmt = stmt.where(
                    or_(DBTextNote.latitude.is_(None), DBTextNote.longitude.is_(None))
                )
        ...
        if "content_contains" in filters:
            stmt = stmt.where(
                DBTextNote.text_content.ilike(f"%{filters['content_contains']}%")
            )
