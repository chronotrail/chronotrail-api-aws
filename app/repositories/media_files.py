...
from app.models.schemas import FileType, MediaFileCreate
...
class MediaFileRepository(BaseRepository[DBMediaFile, MediaFileCreate, None]):
    ...
    async def get_by_user(
        self, db: AsyncSession, user_id: UUID, file_id: UUID
        ...
        stmt = select(DBMediaFile).where(
            and_(DBMediaFile.id == file_id, DBMediaFile.user_id == user_id)
        )
        result = await db.execute(stmt)
        return result.scalars().first()
    ...
    async def get_by_file_type(
        ...
        stmt = (
            ...
            .order_by(DBMediaFile.timestamp.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def get_by_location(
        ...
        stmt = select(DBMediaFile).where(
            and_(
                ...
                DBMediaFile.latitude.between(
                    latitude - lat_delta, latitude + lat_delta
                ),
                DBMediaFile.longitude.between(
    ...
    async def search_by_names(
        ...
        if search_terms:
            ...
            for term in lower_terms:
                conditions.append(
                    func.lower(func.array_to_string(DBMediaFile.names, " ")).contains(
                        term
                    )
                )
    ...
    async def get_total_storage_used(
        self, db: AsyncSession, user_id: UUID, file_type: Optional[FileType] = None
    ) -> int:
        """
        ...
        stmt = select(func.coalesce(func.sum(DBMediaFile.file_size), 0)).where(
            and_(DBMediaFile.user_id == user_id, DBMediaFile.file_size.isnot(None))
        )
    ...
    async def update(
        self, db: AsyncSession, *, db_obj: DBMediaFile, obj_in: None
    ) -> DBMediaFile:
        """
    ...
    def _apply_filters(self, stmt, filters: Dict[str, Any]):
        ...
        if "has_location" in filters:
            if filters["has_location"]:
                ...
                stmt = stmt.where(
                    or_(DBMediaFile.latitude.is_(None), DBMediaFile.longitude.is_(None))
                )
        ...
        if "filename_contains" in filters:
            stmt = stmt.where(
                and_(
                    DBMediaFile.original_filename.isnot(None),
                    DBMediaFile.original_filename.ilike(
                        f"%{filters['filename_contains']}%"
                    ),
                )
            )
