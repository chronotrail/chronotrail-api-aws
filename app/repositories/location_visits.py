...
class LocationVisitRepository(BaseRepository[DBLocationVisit, LocationVisitCreate, LocationVisitUpdate]):
    """
    ...
    async def get_multi(
        ...
        *, 
        ...
        stmt = stmt.offset(skip).limit(limit).order_by(DBLocationVisit.visit_time.desc())
    ...
    async def get_by_user_with_date_range(
        ...
        stmt = stmt.order_by(DBLocationVisit.visit_time.desc()).offset(skip).limit(limit)
    ...
    async def get_by_coordinates(
        ...
        stmt = select(DBLocationVisit).where(
            and_(
                DBLocationVisit.user_id == user_id,
                DBLocationVisit.latitude.between(latitude - lat_delta, latitude + lat_delta),
                DBLocationVisit.longitude.between(longitude - lng_delta, longitude + lng_delta)
            )
        ).order_by(DBLocationVisit.visit_time.desc()).offset(skip).limit(limit)
    ...
    async def count_by_user(
        ...
        Returns:
            Count of matching location visits
        """
        stmt = select(func.count(DBLocationVisit.id)).where(DBLocationVisit.user_id == user_id)
    ...
    async def get_recent_by_user(
        ...
        stmt = select(DBLocationVisit).where(
            DBLocationVisit.user_id == user_id
        ).order_by(DBLocationVisit.visit_time.desc()).limit(limit)
    ...
    def _apply_filters(self, stmt, filters: Dict[str, Any]):
        ...
        if "address_contains" in filters:
            stmt = stmt.where(DBLocationVisit.address.ilike(f"%{filters['address_contains']}%"))
        ...
        if "names_contain" in filters:
            ...
            for term in search_terms:
                conditions.append(
                    func.lower(func.array_to_string(DBLocationVisit.names, ' ')).contains(term.lower())
                )
