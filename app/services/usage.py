...
from app.models.schemas import SubscriptionInfo, UsageStats, User
...
class UsageService:
    ...
    async def get_or_create_daily_usage(
        db: AsyncSession, user_id: UUID, usage_date: date = None
    ) -> DBDailyUsage:
        """
        ...
        stmt = select(DBDailyUsage).where(
            DBDailyUsage.user_id == user_id, DBDailyUsage.usage_date == usage_date
        )
    ...
    async def check_daily_content_limit(
        db: AsyncSession, user: User, content_type: str = "content"
    ) -> bool:
        """
    ...
    async def increment_content_usage(
        db: AsyncSession, user_id: UUID, content_type: str
    ) -> None:
        """
    ...
    async def get_usage_stats(db: AsyncSession, user_id: UUID) -> UsageStats:
        ...
        return UsageStats(
            ...
            daily_limits={
                ...
                "total_content_used": daily_usage.text_notes_count
                + daily_usage.media_files_count,
            },
        )
    ...
    def check_subscription_validity(user: User) -> User:
        ...
        if (
            user.subscription_tier in ["premium", "pro"]
