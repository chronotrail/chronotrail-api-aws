...
from app.models.schemas import (
    ContentType, VectorSearchResult, User, QueryRequest, QueryResponse,
    MediaReference, FileType
)
...
class QueryProcessingService:
    ...
    def _parse_query_intent(self, query: str) -> QueryIntent:
        ...
        for pattern in location_patterns:
            ...
            location_terms.extend([match if isinstance(match, str) else match[0] for match in matches])
        ...
        stop_words = {
            ...
            'could', 'should', 'may', 'might', 'must', 'can', 'what', 'where', 'when', 'why',
        ...
        content_terms = [word for word in words if word not in stop_words and len(word) > 2]
        ...
        if intent.has_media_filter:
            ...
            for term in intent.media_terms:
                if term in ['photo', 'picture', 'image', 'pic']:
                    content_types.extend([ContentType.IMAGE_TEXT, ContentType.IMAGE_DESC])
        ...
        if intent.has_time_filter:
            ...
            if time_range:
                if query_filter.date_range:
                    ...
                    start_date = max(
                        time_range.get("start_date", query_filter.date_range["start_date"]),
                    ...
                    query_filter.date_range = {"start_date": start_date, "end_date": end_date}
                else:
    ...
    async def _perform_search(
        ...
        try:
            ...
            vector_results = self.embedding_service.convert_search_results_to_model(vector_results)
            
            # Log search metrics
            logger.debug(f"Vector search returned {len(vector_results)} results with avg score: "
        ...
        if intent.query_type in ["location", "time", "hybrid"] or intent.has_location_filter or intent.has_time_filter:
            structured_results = await self._search_structured_data(
                user_id, intent, query_filter, db
            )
            logger.debug(f"Structured search returned {len(structured_results)} results")
        ...
        combined_score = self._calculate_combined_score(vector_results, structured_results, intent)
    ...
    def _calculate_combined_score(
        ...
        if vector_results:
            ...
            top_results = sorted(vector_results, key=lambda x: x.score, reverse=True)[:5]
            vector_score = sum(r.score for r in top_results) / len(top_results)
        
        structured_score = 0.0
        if structured_results:
            # Use top 5 results for scoring
            top_results = sorted(structured_results, key=lambda x: x.get("score", 0), reverse=True)[:5]
            structured_score = sum(r.get("score", 0) for r in top_results) / len(top_results)
        
        # Calculate weighted score
        if vector_results and structured_results:
            return (vector_score * vector_weight) + (structured_score * structured_weight)
        elif vector_results:
            return vector_score
    ...
    async def _search_structured_data(
            ...
            try:
                ...
                if query_filter.date_range:
                    ...
                    # Otherwise, search directly with date range
                        ...
                        date_visits = await location_visit_repository.get_by_user_with_date_range(
                            db, user_id,
                            start_date=query_filter.date_range.get("start_date"),
                            end_date=query_filter.date_range.get("end_date"),
                            limit=10
                ...
                for visit in location_visits:
                    ...
                    if visit.names and intent.location_terms:
                        for term in intent.location_terms:
                            if any(term.lower() in name.lower() for name in visit.names):
                                base_score += 0.1
                                break
        ...
        if intent.query_type in ["general", "hybrid"] or intent.has_location_filter or intent.has_time_filter:
            try:
                text_notes = []
                ...
                if intent.content_terms:
                    ...
                    if len(intent.content_terms) > 1:
                        ...
                        for i in range(len(intent.content_terms) - 1):
                            combined_term = f"{intent.content_terms[i]} {intent.content_terms[i+1]}"
                ...
                for note in text_notes:
                    ...
                    if intent.content_terms:
                        ...
                        match_count = sum(1 for term in intent.content_terms if term.lower() in content_lower)
                        if match_count > 0:
                            match_ratio = match_count / len(intent.content_terms)
                            base_score += min(match_ratio * 0.15, 0.15)  # Max 0.15 boost
                    ...
                    results.append({
                        "type": "text_note",
                        "id": str(note.id),
                        "content": note.text_content[:150] + ("..." if len(note.text_content) > 150 else ""),
                        "timestamp": note.timestamp,
                        "location": {
                            "longitude": float(note.longitude) if note.longitude else None,
                            "latitude": float(note.latitude) if note.latitude else None,
                            "address": note.address,
                            "names": note.names
                        } if note.longitude and note.latitude else None,
                        "score": final_score
                    })
            ...
            try:
                ...
                for media in media_files:
                    ...
                    results.append({
                        ...
                        "location": {
                            "longitude": float(media.longitude) if media.longitude else None,
                            "latitude": float(media.latitude) if media.latitude else None,
                            "address": media.address,
                            "names": media.names
                        } if media.longitude and media.latitude else None,
                        "score": final_score
                    })
    ...
    def _rank_and_score_results(
        ...
        for result in search_result.structured_results:
            all_results.append({
                **result,
                "result_source": "structured_search"
            })
        ...
        for result in all_results:
            ...
            if intent.query_type == "location":
                ...
                if time_range and result.get("timestamp"):
                    ...
                    time_match = ((not start_date or result["timestamp"] >= start_date) and
                                 (not end_date or result["timestamp"] <= end_date))
            ...
            if result.get("location") and intent.location_terms:
                ...
                address_match = any(term.lower() in address for term in intent.location_terms) if address else False
                names_match = any(any(term.lower() in name.lower() for name in names) for term in intent.location_terms) if names else False
    ...
    async def _generate_response(
        ...
        if not ranked_results:
            ...
            if session_id:
                try:
                    session = await query_session_repository.get_by_user(db, user.id, UUID(session_id))
        ...
        if result_count > 1:
            ...
            if "when" in query_request.query.lower() or "time" in query_request.query.lower() or "date" in query_request.query.lower():
                if dates:
            ...
            elif "where" in query_request.query.lower() or "location" in query_request.query.lower() or "place" in query_request.query.lower():
                if locations:
            ...
            if len(location_visits) > 1:
                ...
                if len(location_list) > 1:
                    additional_info.append(f"visits to {', '.join(location_list[:-1])} and {location_list[-1]}")
                elif location_list:
        ...
        # Update session
        await query_session_repository.update_context(
            db, session.id, context
        )
