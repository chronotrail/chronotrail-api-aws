...
@router.post(
    ...
    responses={
        201: {"description": "Voice recording uploaded and processed successfully", "model": FileUploadResponse},
...
async def upload_voice(
    ...
    timestamp: Optional[str] = Form(None, description="ISO timestamp when recording was made"),
    longitude: Optional[float] = Form(None, description="Longitude coordinate", ge=-180, le=180),
    latitude: Optional[float] = Form(None, description="Latitude coordinate", ge=-90, le=90),
    address: Optional[str] = Form(None, description="Address or location description", max_length=1000),
    names: Optional[str] = Form(None, description="Comma-separated location names/tags"),
    ...
    try:
        ...
        try:
            ...
            if processed_text and processed_text.strip() and not processed_text.strip().startswith("Audio"):
                ...
                try:
                    await embedding_service.process_and_store_text(
                        ...
                        metadata={
                            ...
                            "confidence_score": getattr(processed_result, 'confidence_score', 0.0),
                            "duration_seconds": getattr(processed_result, 'duration_seconds', None)
                        }
                    )
                except Exception as embedding_error:
                    # Log the error but don't fail the request - the file is still stored
                    logger.error(f"Failed to store voice content in vector database: {str(embedding_error)}")
        ...
        return FileUploadResponse(
            ...
            message=f"Voice recording uploaded and processed successfully. {processed_text[:100]}..." if len(processed_text) > 100 else processed_text,
