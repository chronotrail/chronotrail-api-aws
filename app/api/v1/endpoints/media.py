...
async def get_media_file(
    ...
    try:
        ...
        media_file = await media_file_repository.get_by_user(
            db=db, user_id=current_user.id, file_id=media_id
        )
        ...
        try:
            ...
            if e.status_code == 404:
                ...
                # Other S3 errors
                logger.error(
                    f"Failed to download media file {media_id} from S3: {str(e)}"
                )
                raise HTTPException(
        ...
        if media_file.original_filename:
            headers["Content-Disposition"] = (
                f'inline; filename="{media_file.original_filename}"'
            )
        else:
    ...
    try:
        ...
        return {
            ...
            "location": (
                ...
                if media_file.longitude and media_file.latitude
                else None
            ),
        ...
        # Log unexpected errors
        logger.error(
            f"Unexpected error retrieving media file info {media_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def get_media_download_url(
    ...
    try:
        ...
        # Log unexpected errors
        logger.error(
            f"Unexpected error generating download URL for media file {media_id}: {str(e)}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
