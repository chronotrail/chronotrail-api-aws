...
async def google_oauth_signin(
    request_data: GoogleOAuthRequest, db: AsyncSession = Depends(get_async_db)
) -> TokenResponse:
    """
    ...
    user = await AuthService.get_or_create_user(
        db=db, oauth_provider=OAuthProvider.GOOGLE, oauth_data=google_user_info
    )
...
@router.post(
    ...
    responses={
        ...
        503: {
            "description": "Apple authentication service unavailable",
            "model": ErrorResponse,
        },
    },
)
async def apple_signin(
    request_data: AppleOAuthRequest, db: AsyncSession = Depends(get_async_db)
) -> TokenResponse:
    """
    ...
    # Get or create user in our database
    user = await AuthService.get_or_create_user(
        db=db, oauth_provider=OAuthProvider.APPLE, oauth_data=apple_user_info
    )
...
async def refresh_token(
    refresh_request: RefreshTokenRequest, db: AsyncSession = Depends(get_async_db)
) -> TokenResponse:
    """
