    ...
    def validate_location_consistency(cls, values):
        ...
        if isinstance(values, dict):
            ...
            if (longitude is None) != (latitude is None):
                raise ValueError(
                    "Both longitude and latitude must be provided together or both omitted"
                )
        ...
        if self.original_filename and self.file_type:
            ...
            if self.file_type == FileType.PHOTO and extension:
                try:
                    ...
                    raise ValueError(
                        f"Invalid photo file extension. Allowed extensions: {', '.join(allowed)}"
                    )
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid voice file extension. Allowed extensions: {', '.join(allowed)}"
                    )
...
class FileUploadResponse(BaseModel):
    ...
    processing_status: str = Field(
        default="pending", pattern="^(pending|processing|completed|failed)$"
    )
...
class GoogleOAuthRequest(BaseModel):
    ...
    access_token: str = Field(
        ..., min_length=1, description="Google OAuth access token"
    )
...
class AppleOAuthRequest(BaseModel):
    ...
    identity_token: str = Field(
        ..., min_length=1, description="Apple Sign-In identity token"
    )
