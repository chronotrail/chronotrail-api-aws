...
class User(Base):
    ...
    subscription_tier = Column(String(50), nullable=False, default="free")  # 'free', 'premium', 'pro'
    subscription_expires_at = Column(DateTime, nullable=True)
    created_at = Column(DateTime, nullable=False, default=func.now())
    updated_at = Column(DateTime, nullable=False, default=func.now(), onupdate=func.now())
    
    # Relationships
    location_visits = relationship("LocationVisit", back_populates="user", cascade="all, delete-orphan")
    text_notes = relationship("TextNote", back_populates="user", cascade="all, delete-orphan")
...
class QuerySession(Base):
    ...
    id = Column(PostgresUUID(as_uuid=True), primary_key=True, default=uuid4)
    user_id = Column(PostgresUUID(as_uuid=True), ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    session_context = Column(JSON, nullable=True)  # Stores conversation context and referenced media
    last_query = Column(Text, nullable=True)
    created_at = Column(DateTime, nullable=False, default=func.now())
    updated_at = Column(DateTime, nullable=False, default=func.now(), onupdate=func.now())
    expires_at = Column(DateTime, nullable=False, default=lambda: func.now() + func.make_interval(hours=1))
