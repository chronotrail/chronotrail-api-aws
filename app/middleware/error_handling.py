...
from app.core.logging import get_logger, add_request_context, LoggingContext
from app.core.exceptions import create_error_response, ChronoTrailError
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if self.log_response_body and hasattr(response, 'body'):
                try:
                    # Only log first 1000 characters to avoid huge logs
                    body_preview = response.body.decode('utf-8')[:1000] if response.body else None
                    if body_preview:
            ...
            if response.status_code >= 500:
                request_logger.error("Request completed with server error", **response_data)
            elif response.status_code >= 400:
                request_logger.warning("Request completed with client error", **response_data)
            else:
