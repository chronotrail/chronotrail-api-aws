    ...
    def _format_context_for_claude(self, context_items: List[Dict[str, Any]]) -> str:
        ...
        for i, item in enumerate(context_items, 1):
            ...
            if item_type == "location_visit":
                ...
                file_type_str = "Photo" if file_type == FileType.PHOTO else "Voice Recording"
...
"""
        ...
        try:
            ...
            if self.model_id != FALLBACK_MODEL_ID:
                ...
                try:
                    ...
                    return fallback_service.generate_response(query, context_items, media_references)
    ...
    def _generate_fallback_response(
        ...
        """
        # Count items by type
        location_visits = [item for item in context_items if item.get("type") == "location_visit"]
        text_notes = [item for item in context_items if item.get("type") == "text_note"]
        media_files = [item for item in context_items if item.get("type") == "media_file"]
        ...
        if media_files:
            ...
            if photos and voice:
                type_descriptions.append(f"{photos} photos and {voice} voice recordings")
            elif photos:
