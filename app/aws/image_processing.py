...
class ImageProcessingService:
    ...
    def describe_image_sync(self, image_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            label_response = rekognition_client.detect_labels(
                Image={"Bytes": image_data}, MaxLabels=10, MinConfidence=70
            )
            ...
            labels = [
                {
                    ...
                    "parents": [
                        parent.get("Name", "") for parent in label.get("Parents", [])
                    ],
                }
            ...
            text_response = rekognition_client.detect_text(Image={"Bytes": image_data})
    ...
    async def describe_image(self, image_data: bytes) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Failed to analyze image: {str(e)}")
            raise RekognitionError(
                message=f"Failed to analyze image: {str(e)}", operation="describe_image"
            )

    async def process_image(
        self, file: Union[UploadFile, bytes], original_id: Optional[UUID] = None
    ) -> ProcessedPhoto:
        """
        ...
        try:
            ...
            result.detected_objects = [
                label["name"] for label in image_analysis.get("labels", [])
            ]
        ...
        if (
            ...
            or result.processed_text.strip() == "Image contains:"
        ):
