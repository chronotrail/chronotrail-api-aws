...
class TestAuthenticationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_google_oauth_signin_complete_flow(
        ...
        with (
            patch(
                "app.services.auth.AuthService.verify_google_token",
                return_value=mock_google_user,
            ),
            patch(
                "app.services.auth.AuthService.get_or_create_user", return_value=db_user
            ),
            patch(
                "app.services.auth.AuthService.create_tokens_for_user"
            ...
            response = await client.post(
                "/api/v1/auth/google", json={"access_token": "google_oauth_token"}
            )
    ...
    @pytest.mark.asyncio
    async def test_apple_signin_complete_flow(
        self, client: AsyncClient, mock_db: AsyncSession
    ):
        ...
        with (
            patch(
                "app.services.auth.AuthService.verify_apple_token",
            ...
            # Make request
            response = await client.post(
                "/api/v1/auth/apple", json={"identity_token": "apple_identity_token"}
            )
    ...
    @pytest.mark.asyncio
    async def test_refresh_token_complete_flow(
        self, client: AsyncClient, mock_db: AsyncSession
    ):
    ...
    async def test_get_current_user_profile(
        self, client: AsyncClient, mock_auth_user: Dict[str, Any]
    ):
...
class TestQueryIntegration:
    ...
    async def test_query_complete_processing_flow(
        ...
        from app.models.schemas import MediaReference, QueryResponse
        ...
        with patch(
            ...
            response = await client.post(
                "/api/v1/query",
                json={"query": "What did I do yesterday?", "session_id": "session_123"},
                headers={"Authorization": f"Bearer {mock_auth_user['token']}"},
            )
...
class TestMediaRetrievalIntegration:
    ...
    async def test_media_download_url_generation(
        ...
        mock_download_url = (
            "https://s3.amazonaws.com/bucket/test/path/image.jpg?signature=abc123"
        )
        ...
        with (
            ...
            patch(
                "app.aws.storage.file_storage_service.generate_download_url",
                return_value=mock_download_url,
            ),
        ):
...
class TestUsageAndSubscriptionIntegration:
    ...
    async def test_usage_stats_retrieval_complete_flow(
        ...
        with patch(
            "app.services.usage.UsageService.get_usage_stats",
    ...
    async def test_subscription_info_retrieval_complete_flow(
        ...
        mock_subscription_info = SubscriptionInfo(
            ...
            expires_at=None,
            daily_limits={"content_limit": 3, "query_limit": 10},
        ...
        with (
            patch(
                ...
                return_value=mock_auth_user["user"],
            ),
            patch(
                "app.services.usage.UsageService.get_subscription_info",
...
class TestAuthorizationIntegration:
    ...
    @pytest.mark.asyncio
    async def test_unauthorized_access_to_protected_endpoints(
    ...
    async def test_cross_user_data_access_prevention(
        ...
        with (
            ...
            patch(
                "app.repositories.media_files.media_file_repository.get",
                return_value=user2_media,
            ),
...
class TestFileUploadWorkflows:
    ...
    async def test_voice_upload_with_file_size_validation(
        ...
        with patch(
            "app.middleware.usage_validation.validate_file_size",
