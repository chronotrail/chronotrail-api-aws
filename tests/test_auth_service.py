...
from app.models.schemas import User, UserCreate, UserUpdate, OAuthProvider
...
async def test_verify_apple_token_key_not_found(mock_apple_jwk):
    ...
    with patch("httpx.AsyncClient", return_value=mock_client), \
         patch("jose.jwt.get_unverified_header", return_value={"kid": "non_matching_key_id"}):
...
async def test_get_or_create_user_update(mock_google_user_info, mock_user):
    ...
    with patch("app.services.auth.get_user_by_oauth", return_value=outdated_user), \
...
async def test_get_or_create_user_new(mock_google_user_info):
    ...
    with patch("app.services.auth.get_user_by_oauth", return_value=None), \
...
async def test_get_or_create_user_missing_info():
    ...
    with pytest.raises(HTTPException) as exc_info:
        await AuthService.get_or_create_user(
            mock_db,
            "google",
            incomplete_data
        )
