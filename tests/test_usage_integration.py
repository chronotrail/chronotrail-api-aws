...
from app.models.schemas import OAuthProvider, SubscriptionTier, User
...
class TestUsageIntegration:
    ...
    def test_tier_hierarchy_consistency(self):
        ...
        assert (
            premium_limits["daily_content_limit"] > free_limits["daily_content_limit"]
        )
        assert premium_limits["daily_query_limit"] > free_limits["daily_query_limit"]
        assert (
            premium_limits["query_history_months"] > free_limits["query_history_months"]
        )
        ...
        assert (
            pro_limits["daily_content_limit"] == -1
            or pro_limits["daily_content_limit"] > premium_limits["daily_content_limit"]
        )
        assert (
            pro_limits["daily_query_limit"] == -1
            or pro_limits["daily_query_limit"] > premium_limits["daily_query_limit"]
        )
        assert (
            pro_limits["query_history_months"] == -1
            or pro_limits["query_history_months"]
            > premium_limits["query_history_months"]
        )
