...
class TestUsageService:
    ...
    async def test_get_or_create_daily_usage_new_record(self, mock_db):
        ...
        with patch.object(
            UsageService, "get_or_create_daily_usage", return_value=expected_usage
        ) as mock_method:
            result = await UsageService.get_or_create_daily_usage(
                mock_db, user_id, usage_date
            )
    ...
    async def test_get_or_create_daily_usage_existing_record(self, mock_db):
        ...
        with patch.object(
            UsageService, "get_or_create_daily_usage", return_value=existing_usage
        ) as mock_method:
            result = await UsageService.get_or_create_daily_usage(
                mock_db, user_id, usage_date
            )
    ...
    async def test_check_daily_content_limit_within_limit(self, mock_db, free_user):
        ...
        with patch.object(
            UsageService, "get_or_create_daily_usage", return_value=daily_usage
        ):
    ...
    async def test_get_usage_stats(self, mock_db, free_user):
        ...
        with patch.object(
            UsageService, "get_usage_stats", return_value=expected_stats
        ) as mock_method:
    ...
    def test_validate_query_date_range_exceeds_limit(self, free_user):
        ...
        assert "Query date range exceeds subscription limits" in str(
    ...
    def test_validate_file_size_within_limit(self, free_user):
        ...
        result = UsageService.validate_file_size(
            free_user, 3.0
        )  # 3MB for free tier (limit is 5MB)
        assert result is True

    def test_validate_file_size_exceeds_limit(self, free_user):
        """Test file size validation exceeding limits."""
        with pytest.raises(HTTPException) as exc_info:
            UsageService.validate_file_size(
                free_user, 10.0
            )  # 10MB exceeds 5MB limit for free tier
