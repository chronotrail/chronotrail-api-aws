...
from app.models.schemas import RefreshTokenRequest, TokenResponse, User
...
class TestJWTTokenValidation:
    ...
    def test_decode_token_wrong_secret(self, sample_user):
        ...
        wrong_secret_token = jwt.encode(
            token_data, "wrong-secret", algorithm=settings.ALGORITHM
        )
...
class TestRefreshTokenFunctionality:
    ...
    async def test_refresh_access_token_success(self, sample_user):
        ...
        with patch("app.services.auth.get_user_by_id", return_value=sample_user):
            ...
            new_token_response = await AuthService.refresh_access_token(
                mock_db, token_response.refresh_token
            )
            ...
            assert (
                new_token_response.refresh_token is None
            )  # Only new access token returned
    ...
    async def test_refresh_with_expired_refresh_token(self, sample_user):
        ...
        expired_refresh_token = jwt.encode(
            expired_refresh_data, settings.SECRET_KEY, algorithm=settings.ALGORITHM
        )
...
class TestAuthenticationMiddleware:
    ...
    async def test_get_current_user_with_refresh_token_fails(self, sample_user):
        ...
        credentials = HTTPAuthorizationCredentials(
            scheme="Bearer", credentials=token_response.refresh_token
        )
...
class TestTokenSecurity:
    ...
    def test_refresh_token_has_unique_jti(self, sample_user):
        ...
        payload1 = jwt.decode(
            token1.refresh_token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM]
        )

        payload2 = jwt.decode(
            token2.refresh_token, settings.SECRET_KEY, algorithms=[settings.ALGORITHM]
        )
    ...
    async def test_token_type_validation(self, sample_user):
        ...
        invalid_data = {"sub": str(sample_user.id), "email": sample_user.email}

        invalid_token = jwt.encode(
            invalid_data, settings.SECRET_KEY, algorithm=settings.ALGORITHM
        )
