...
class TestTextNoteRepository:
    ...
    async def test_create_text_note(self, async_db_session: AsyncSession, test_user, sample_text_note_data):
        """Test creating a text note."""
        note = await test_text_note_repository.create(
            async_db_session, 
            obj_in=sample_text_note_data, 
    ...
    async def test_create_text_note_no_location(self, async_db_session: AsyncSession, test_user, sample_text_note_no_location):
        """Test creating a text note without location data."""
        note = await test_text_note_repository.create(
            async_db_session, 
            obj_in=sample_text_note_no_location, 
            user_id=test_user.id
        )
    ...
    async def test_get_text_note(self, async_db_session: AsyncSession, test_user, sample_text_note_data):
        ...
        retrieved_note = await test_text_note_repository.get(async_db_session, created_note.id)
    ...
    async def test_get_by_user(self, async_db_session: AsyncSession, test_user, test_user_2, sample_text_note_data):
    ...
    async def test_get_by_user_with_date_range(self, async_db_session: AsyncSession, test_user):
        ...
        await test_text_note_repository.create(async_db_session, obj_in=note1_data, user_id=test_user.id)
        await test_text_note_repository.create(async_db_session, obj_in=note2_data, user_id=test_user.id)
        await test_text_note_repository.create(async_db_session, obj_in=note3_data, user_id=test_user.id)
    ...
    async def test_get_by_location(self, async_db_session: AsyncSession, test_user):
        ...
        await test_text_note_repository.create(async_db_session, obj_in=note2_data, user_id=test_user.id)
        await test_text_note_repository.create(async_db_session, obj_in=note3_data, user_id=test_user.id)
        await test_text_note_repository.create(async_db_session, obj_in=note4_data, user_id=test_user.id)
        ...
        assert len(notes) == 2
        assert all("Near target" in note.text_content or "Also near target" in note.text_content for note in notes)
    ...
    async def test_get_with_location(self, async_db_session: AsyncSession, test_user):
        ...
        notes = await test_text_note_repository.get_with_location(async_db_session, test_user.id)
    ...
    async def test_count_by_user(self, async_db_session: AsyncSession, test_user, test_user_2):
        ...
        for i in range(3):
            note_data = TextNoteCreate(
                text_content=f"User 1 Note {i}",
                timestamp=now - timedelta(hours=i)
            )
            await test_text_note_repository.create(async_db_session, obj_in=note_data, user_id=test_user.id)
        
        # Create notes for user 2
        for i in range(2):
            note_data = TextNoteCreate(
                text_content=f"User 2 Note {i}",
                timestamp=now - timedelta(hours=i)
            )
            await test_text_note_repository.create(async_db_session, obj_in=note_data, user_id=test_user_2.id)
        
        # Count for user 1
        count1 = await test_text_note_repository.count_by_user(async_db_session, test_user.id)
        assert count1 == 3
        
        # Count for user 2
        count2 = await test_text_note_repository.count_by_user(async_db_session, test_user_2.id)
        assert count2 == 2
    ...
    async def test_get_recent_by_user(self, async_db_session: AsyncSession, test_user):
        ...
        notes = await test_text_note_repository.get_recent_by_user(async_db_session, test_user.id, limit=3)
    ...
    async def test_delete_by_user(self, async_db_session: AsyncSession, test_user, test_user_2, sample_text_note_data):
        ...
        still_exists = await test_text_note_repository.get(async_db_session, another_note.id)
    ...
    async def test_user_isolation(self, async_db_session: AsyncSession, test_user, test_user_2):
        ...
        note1_data = TextNoteCreate(
            text_content="User 1 note",
            timestamp=datetime.utcnow() - timedelta(hours=1)
        )
        note2_data = TextNoteCreate(
            text_content="User 2 note",
            timestamp=datetime.utcnow() - timedelta(hours=2)
        )
        
        note1 = await test_text_note_repository.create(async_db_session, obj_in=note1_data, user_id=test_user.id)
        note2 = await test_text_note_repository.create(async_db_session, obj_in=note2_data, user_id=test_user_2.id)
