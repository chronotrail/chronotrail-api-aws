...
class TestGetMediaFile:
    ...
    @patch("app.api.v1.endpoints.media.get_current_active_user")
    @patch("app.api.v1.endpoints.media.media_file_repository")
    def test_get_media_file_not_found(self, mock_repository, mock_get_user, test_user):
    ...
    def test_get_media_file_unauthorized_access(
        self, mock_repository, mock_get_user, test_user, other_user, test_media_file
    ):
...
class TestGetMediaFileInfo:
    ...
    def test_get_media_file_info_success(
        self, mock_repository, mock_get_user, test_user, test_voice_file
    ):
    ...
    def test_get_media_file_info_no_location(
        self, mock_repository, mock_get_user, test_user, test_media_file
    ):
    ...
    def test_get_media_file_info_not_found(
        self, mock_repository, mock_get_user, test_user
    ):
...
class TestContentTypeDetection:
    ...
    def test_photo_content_types(
        self, mock_storage_service, mock_repository, mock_get_user, test_user
    ):
...
class TestFilenameGeneration:
    ...
    def test_generated_filename_photo(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert (
            'filename="photo_20240115_103045.jpg"'
            in response.headers["content-disposition"]
        )
    ...
    def test_generated_filename_voice(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert (
            'filename="voice_20240115_144530.mp3"'
            in response.headers["content-disposition"]
        )
