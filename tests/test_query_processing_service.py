...
class TestQueryFilter:
    ...
    @pytest.mark.asyncio
    async def test_create_filter_with_media_terms(
    ...
    @pytest.mark.asyncio
    async def test_create_filter_with_location_terms(
        self, query_service, test_user, mock_db
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_filter_with_subscription_limits(
        self, query_service, test_user, mock_db
    ):
...
class TestVectorSearch:
    ...
    async def test_vector_search_failure(
        ...
        # Mock search failure
        mock_embedding_service.search_by_text_async.side_effect = Exception(
            "Search failed"
        )
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        with patch(
                            ...
                            mock_usage.can_make_query.assert_called_once_with(
                                test_user.id, mock_db
                            )
                            mock_usage.increment_query_usage.assert_called_once_with(
                                test_user.id, mock_db
                            )
    ...
    async def test_process_query_usage_limit_exceeded(
        ...
        with patch("app.services.query.usage_service") as mock_usage:
            ...
            assert (
                ...
                or "limit exceeded" in str(exc_info.value).lower()
            )
