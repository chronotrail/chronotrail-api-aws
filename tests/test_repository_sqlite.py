...
class LocationVisitModelRepository:
    ...
    async def get_by_user_with_date_range(
        ...
        stmt = (
            stmt.order_by(LocationVisitModel.visit_time.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def search_by_address(
        ...
        stmt = (
            ...
            .order_by(LocationVisitModel.visit_time.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def count_by_user(
        ...
        stmt = select(func.count(LocationVisitModel.id)).where(
            LocationVisitModel.user_id == str(user_id)
        )
    ...
    async def get_recent_by_user(
        ...
        stmt = (
            select(LocationVisitModel)
            .where(LocationVisitModel.user_id == str(user_id))
            .order_by(LocationVisitModel.visit_time.desc())
            .limit(limit)
        )
    ...
    async def get_by_coordinates(
        ...
        stmt = (
            ...
            .where(
                and_(
                    LocationVisitModel.user_id == str(user_id),
                    LocationVisitModel.latitude.between(
                        latitude - lat_delta, latitude + lat_delta
                    ),
                    LocationVisitModel.longitude.between(
