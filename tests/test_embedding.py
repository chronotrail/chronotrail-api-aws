...
class TestEmbeddingService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.sample_metadata = {"file_type": "text", "original_filename": "test.txt"}
    ...
    def test_generate_embeddings_error(self, mock_get_bedrock_client):
        ...
        mock_client.invoke_model.side_effect = ClientError(
            {"Error": {"Code": "ModelError", "Message": "Model error"}}, "InvokeModel"
        )
        mock_get_bedrock_client.return_value = mock_client
    ...
    def test_ensure_index_exists_new(self, mock_get_opensearch_client):
        ...
        mock_client.indices.create.assert_called_once_with(
            index=self.test_index, body=OPENSEARCH_MAPPING
        )
    ...
    def test_search_by_vector(self, mock_get_opensearch_client):
        ...
        user_term = next(
            (
                ...
                if "term" in term and "user_id" in term["term"]
            ),
            None,
        )
        ...
        content_filter = next(
            (
                term
                for term in query["must"]
                if "terms" in term and "content_type" in term["terms"]
            ),
            None,
        )
        self.assertIsNotNone(content_filter)
        self.assertEqual(
            set(content_filter["terms"]["content_type"]), {"note", "location_desc"}
        )
    ...
    def test_delete_document(self, mock_get_opensearch_client):
        ...
        mock_client.delete.assert_called_once_with(
            index=self.test_index, id="doc1", refresh=True
        )
...
class TestAsyncEmbeddingService:
    ...
    def sample_data(self):
        ...
        return {
            ...
            "metadata": {"file_type": "text", "original_filename": "test.txt"},
        }

    @patch("app.aws.embedding.EmbeddingService.generate_embeddings")
    async def test_generate_embeddings_async(
        self, mock_generate_embeddings, service, sample_data
    ):
    ...
    async def test_index_document_async(
        self, mock_index_document, service, sample_data
    ):
    ...
    @patch("app.aws.embedding.EmbeddingService.search_by_vector")
    async def test_search_by_vector_async(
        self, mock_search_by_vector, service, sample_data
    ):
    ...
    async def test_search_by_text_async(
        self,
        mock_search_by_vector_async,
        mock_generate_embeddings_async,
        service,
        sample_data,
    ):
    ...
    async def test_process_and_store_text(
        self,
        mock_index_document_async,
        mock_generate_embeddings_async,
        service,
        sample_data,
    ):
