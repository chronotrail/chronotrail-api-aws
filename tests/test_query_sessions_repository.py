...
def sample_session_context():
    ...
    return {
        "conversation_history": [
            ...
            {"role": "assistant", "content": "Based on your data, you visited a coffee shop."}
        ],
        "media_references": [
            {
                "media_id": str(uuid4()),
...
class TestQuerySessionRepository:
    ...
    async def test_create_query_session(self, async_db_session: AsyncSession, test_user, sample_session_context):
    ...
    async def test_create_query_session_minimal(self, async_db_session: AsyncSession, test_user):
    ...
    async def test_get_query_session(self, async_db_session: AsyncSession, test_user, sample_session_context):
        ...
        retrieved_session = await test_query_session_repository.get(async_db_session, created_session.id)
    ...
    async def test_get_expired_session_returns_none(self, async_db_session: AsyncSession, test_user):
        ...
        if isinstance(created_session.session_context, dict):
            created_session.session_context = json.dumps(created_session.session_context)
    ...
    async def test_get_by_user(self, async_db_session: AsyncSession, test_user, test_user_2, sample_session_context):
    ...
    async def test_get_active_sessions_by_user(self, async_db_session: AsyncSession, test_user, test_user_2):
        """Test getting active sessions for a user."""
        # Create sessions for user 1
        for i in range(3):
            session_data = QuerySessionCreate(
                user_id=test_user.id,
                session_context={"query_count": i}
            )
            await test_query_session_repository.create(async_db_session, obj_in=session_data)
        ...
        user1_sessions = await test_query_session_repository.get_active_sessions_by_user(
        ...
        user2_sessions = await test_query_session_repository.get_active_sessions_by_user(
    ...
    async def test_update_query_session(self, async_db_session: AsyncSession, test_user):
    ...
    async def test_update_context_wrong_user(self, async_db_session: AsyncSession, test_user, test_user_2):
    ...
    async def test_get_media_references(self, async_db_session: AsyncSession, test_user):
        ...
        session_data = QuerySessionCreate(
            user_id=test_user.id,
            session_context={"media_references": media_refs}
        )
    ...
    async def test_get_media_references_empty(self, async_db_session: AsyncSession, test_user):
    ...
    async def test_cleanup_expired_sessions(self, async_db_session: AsyncSession, test_user):
        ...
        for i in range(2):
            session_data = QuerySessionCreate(user_id=test_user.id)
            expired_session = await test_query_session_repository.create(async_db_session, obj_in=session_data)
            
            # Manually expire the session
            expired_session.expires_at = datetime.utcnow() - timedelta(minutes=1)
            # Convert session_context back to JSON string for SQLite
            if isinstance(expired_session.session_context, dict):
                expired_session.session_context = json.dumps(expired_session.session_context)
            async_db_session.add(expired_session)
            await async_db_session.commit()
        
        # Cleanup expired sessions
        deleted_count = await test_query_session_repository.cleanup_expired_sessions(async_db_session)
        
        assert deleted_count == 2
        
        # Verify active sessions still exist
        active_sessions = await test_query_session_repository.get_active_sessions_by_user(
            async_db_session, test_user.id
        )
        assert len(active_sessions) == 3
    
    async def test_cleanup_old_sessions_by_user(self, async_db_session: AsyncSession, test_user):
        """Test cleaning up old sessions for a user."""
        # Create 5 sessions with different update times
        sessions = []
        for i in range(5):
            session_data = QuerySessionCreate(user_id=test_user.id)
            session = await test_query_session_repository.create(async_db_session, obj_in=session_data)
        ...
        deleted_count = await test_query_session_repository.cleanup_old_sessions_by_user(
        ...
        remaining_sessions = await test_query_session_repository.get_active_sessions_by_user(
            async_db_session, test_user.id
        )
        assert len(remaining_sessions) == 3
    
    async def test_count_active_by_user(self, async_db_session: AsyncSession, test_user, test_user_2):
        ...
        expired_session = await test_query_session_repository.create(async_db_session, obj_in=expired_session_data)
        expired_session.expires_at = datetime.utcnow() - timedelta(minutes=1)
        # Convert session_context back to JSON string for SQLite
        if isinstance(expired_session.session_context, dict):
            expired_session.session_context = json.dumps(expired_session.session_context)
        ...
        count1 = await test_query_session_repository.count_active_by_user(async_db_session, test_user.id)
        assert count1 == 3  # Expired session not counted
        
        count2 = await test_query_session_repository.count_active_by_user(async_db_session, test_user_2.id)
        assert count2 == 2
    
    async def test_get_session_statistics(self, async_db_session: AsyncSession, test_user):
        ...
        stats = await test_query_session_repository.get_session_statistics(async_db_session, test_user.id)
    ...
    async def test_delete_by_user(self, async_db_session: AsyncSession, test_user, test_user_2):
        ...
        still_exists = await test_query_session_repository.get(async_db_session, another_session.id)
        assert still_exists is not None
    
    async def test_user_isolation(self, async_db_session: AsyncSession, test_user, test_user_2):
        """Test that users can only access their own sessions."""
        # Create sessions for both users
        session1_data = QuerySessionCreate(
            user_id=test_user.id,
            session_context={"user": "user1"}
        )
        session2_data = QuerySessionCreate(
            user_id=test_user_2.id,
            session_context={"user": "user2"}
        )
        
        session1 = await test_query_session_repository.create(async_db_session, obj_in=session1_data)
        session2 = await test_query_session_repository.create(async_db_session, obj_in=session2_data)
    ...
    async def test_filters_in_get_multi(self, async_db_session: AsyncSession, test_user):
        ...
        media_sessions = await test_query_session_repository.get_multi(
            async_db_session,
            filters={"has_media_references": True}
        )
        ...
        query_sessions = await test_query_session_repository.get_multi(
            async_db_session,
            filters={"last_query_contains": "test"}
        )
