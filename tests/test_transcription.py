...
from app.aws.exceptions import TranscribeError, S3Error, FileProcessingError
...
def mock_transcribe_response():
    ...
    return {
        ...
        'MediaFormat': 'mp3',
        'Media': {
            'MediaFileUri': 's3://test-bucket/transcribe/test-job.mp3'
        },
        'Transcript': {
...
class TestTranscriptionService:
    ...
    async def test_wait_for_transcription_job_timeout(
        ...
        # Set up mocks
        mock_time.side_effect = [0, 301]  # First call returns 0, second call returns 301 (> 300 timeout)
        mock_get_transcription_job.return_value = {
            'TranscriptionJobStatus': 'IN_PROGRESS'
        }
        ...
        with pytest.raises(TranscribeError) as excinfo:
            await transcription_service.wait_for_transcription_job('test-job', timeout=300)
    ...
    def test_extract_transcript_text(
        ...
        # Extract text
        text, confidence = transcription_service.extract_transcript_text(mock_transcript_data)
        
        # Verify results
        assert text == "This is a test transcription."
        assert confidence == pytest.approx(0.97, abs=0.01)  # Average of confidence scores
    ...
    async def test_transcribe_audio_file_with_upload_file(
        ...
        mock_extract_transcript_text.return_value = ("This is a test transcription.", 0.97)
