...
from app.core.exceptions import ValidationError as ChronoValidationError
from app.core.exceptions import (
...
class TestChronoTrailExceptions:
    ...
    def test_validation_error(self):
        ...
        error = ChronoValidationError(
            message="Invalid data", details={"field": "required"}
        )
...
class TestErrorResponseCreation:
    ...
    def test_create_error_response_aws_error(self):
        ...
        error = S3Error(
            "Access denied", operation="upload_file", error_code="AccessDenied"
        )
...
class TestLoggingIntegration:
    ...
    def test_logging_context_manager(self, mock_get_logger):
        ...
        with LoggingContext(
            mock_logger, user_id="test_user", operation="test_op"
        ) as ctx_logger:
            assert ctx_logger == mock_logger
            mock_logger.bind.assert_called_once_with(
                user_id="test_user", operation="test_op"
            )
    ...
    def test_with_logging_decorator(self, mock_get_logger):
        ...
        )  # At least function call and result logs
