...
from app.aws.exceptions import FileProcessingError, RekognitionError, TextractError
...
def mock_textract_response():
    ...
    return {
        "Blocks": [
            {"BlockType": "LINE", "Text": "This is a test", "Confidence": 99.5},
            {"BlockType": "LINE", "Text": "OCR extraction", "Confidence": 98.7},
        ]
    }
...
def mock_rekognition_text_response():
    ...
    return {
        "TextDetections": [
            ...
            {"DetectedText": "WORLD", "Type": "LINE", "Confidence": 98.5},
        ]
    }
...
class TestImageProcessingService:
    ...
    def test_extract_text_from_image_sync(
        self, mock_get_textract_client, mock_textract_response, image_processing_service
    ):
    ...
    def test_extract_text_from_image_sync_error(
        self, mock_get_textract_client, image_processing_service
    ):
    ...
    async def test_process_image_textract_failure(
        ...
        mock_extract_text.side_effect = TextractError(
            message="Textract processing failed", operation="extract_text_from_image"
        )
        mock_describe_image.return_value = {
            "labels": [
                {"name": "Person", "confidence": 99.2, "parents": []},
    ...
    async def test_process_image_rekognition_failure(
        ...
        mock_describe_image.side_effect = RekognitionError(
            message="Rekognition processing failed", operation="describe_image"
        )
    ...
    @pytest.mark.asyncio
    async def test_process_image_with_bytes(self, image_processing_service):
        """Test processing an image provided as bytes."""
        # Create test image bytes
        image_bytes = b"test image bytes"

        # Mock the extract_text_from_image and describe_image methods
        image_processing_service.extract_text_from_image = AsyncMock(
            return_value="Extracted text"
        )
        image_processing_service.describe_image = AsyncMock(
            return_value={
        ...
        # Verify mocks were called correctly
        image_processing_service.extract_text_from_image.assert_called_once_with(
            image_bytes
        )
