...
class TestLocationVisitsEndpoints:
    ...
    def test_create_location_visit_success(
        ...
        with (
            ...
            patch(
                ...
                return_value=sample_db_location_visit,
            ) as mock_create,
        ):
            ...
            assert float(data["longitude"]) == float(
                sample_location_visit_create.longitude
            )
            assert float(data["latitude"]) == float(
                sample_location_visit_create.latitude
            )
    ...
    def test_create_location_visit_future_time(self, client, mock_db, sample_user):
    ...
    def test_get_location_visits_success(
        self, client, mock_db, sample_user, sample_db_location_visit
    ):
        ...
        with (
            ...
            response = client.get(
                "/api/v1/locations", headers={"Authorization": "Bearer fake_token"}
            )
    ...
    def test_get_location_visits_invalid_date_range(self, client, mock_db, sample_user):
    ...
    def test_update_location_visit_not_found(self, client, mock_db, sample_user):
    ...
    def test_update_location_visit_invalid_names(self, client, mock_db, sample_user):
        ...
        with patch(
            ...
            response = client.put(
                f"/api/v1/locations/{visit_id}",
                json={"names": ["", "  ", "valid_name"]},  # Empty and whitespace names
                headers={"Authorization": "Bearer fake_token"},
            )
    ...
    def test_get_location_visit_by_id_success(
        ...
        with (
            ...
            patch(
                "app.repositories.location_visits.location_visit_repository.get_by_user",
                return_value=sample_db_location_visit,
            ) as mock_get,
        ):
    ...
    def test_get_location_visit_by_id_not_found(self, client, mock_db, sample_user):
    ...
    def test_location_visits_require_authentication(self, client):
        ...
        visit_id = uuid4()
        response = client.put(
            f"/api/v1/locations/{visit_id}", json={"description": "test"}
        )
        assert response.status_code in [
            status.HTTP_401_UNAUTHORIZED,
            status.HTTP_403_FORBIDDEN,
    ...
    def test_subscription_validity_check(self, client, mock_db, sample_user):
        ...
        with patch(
            "app.services.usage.UsageService.check_subscription_validity",
            side_effect=HTTPException(status_code=403, detail="Subscription expired"),
        ) as mock_check:
    ...
    def test_query_date_range_validation(self, client, mock_db, sample_user):
        ...
        with (
            ...
            patch(
                ...
                side_effect=HTTPException(
                    status_code=403, detail="Date range exceeds limits"
                ),
            ) as mock_validate,
        ):
