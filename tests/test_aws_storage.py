...
class TestFileStorageService:
    ...
    async def test_upload_file(
        ...
        assert (
            result["s3_url"]
            == "https://test-bucket.s3.amazonaws.com/user-123/photo/2023/01/01/test-uuid.jpg"
        )
    ...
    async def test_upload_file_validation_failure(
        self, mock_validate_file, mock_upload_file, storage_service
    ):
        ...
        with pytest.raises(HTTPException) as excinfo:
            await storage_service.upload_file(
                file=mock_upload_file, user_id="user-123", file_type="photo"
            )
    ...
    async def test_download_file(
        self, mock_download_file_from_s3, mock_check_file_exists, storage_service
    ):
        """Test file download."""
        # Set up mocks
    ...
    async def test_download_file_not_found(
        self, mock_check_file_exists, storage_service
    ):
    ...
    async def test_delete_file(self, mock_delete_file_from_s3, storage_service):
        """Test file deletion."""
        # Set up mock
    ...
    def test_generate_download_url(
        self, mock_get_s3_client, mock_check_file_exists, storage_service
    ):
        ...
        result = storage_service.generate_download_url(
            s3_key="test-key", expiration=1800, file_name="download.jpg"
        )
    ...
    @patch("app.aws.storage.get_s3_client")
    def test_generate_upload_url(self, mock_get_s3_client, storage_service):
    ...
    @patch("app.aws.storage.check_file_exists")
    def test_check_file_exists(self, mock_check_file_exists, storage_service):
