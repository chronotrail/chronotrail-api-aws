...
class QuerySessionModelRepository:
    ...
    async def create(
        ...
        context_json = (
            json.dumps(obj_in.session_context) if obj_in.session_context else "{}"
        )
        ...
        *,
        ...
        with db.no_autoflush:
            ...
            stmt = (
                stmt.offset(skip)
                .limit(limit)
                .order_by(QuerySessionModel.updated_at.desc())
            )
    ...
    async def update(
        self, db: AsyncSession, *, db_obj: QuerySessionModel, obj_in: QuerySessionUpdate
    ) -> QuerySessionModel:
        ...
        if (
            "session_context" in update_data
            and update_data["session_context"] is not None
        ):
    ...
    async def cleanup_old_sessions_by_user(
        ...
        with db.no_autoflush:
            ...
            keep_stmt = (
                select(QuerySessionModel.id)
                .where(QuerySessionModel.user_id == str(user_id))
                .order_by(QuerySessionModel.updated_at.desc())
                .limit(keep_count)
            )
    ...
    async def get_session_statistics(
        ...
        with db.no_autoflush:
            ...
            if user_id:
                base_filter = and_(
                    base_filter, QuerySessionModel.user_id == str(user_id)
                )
            ...
            if user_id:
                expired_filter = and_(
                    expired_filter, QuerySessionModel.user_id == str(user_id)
                )

            expired_stmt = select(func.count(QuerySessionModel.id)).where(
                expired_filter
            )
            ...
            duration_stmt = select(
                func.avg(
                    func.julianday(QuerySessionModel.updated_at)
                    - func.julianday(QuerySessionModel.created_at)
                )
                * 24
            ...
            return {
                ...
                "average_duration_minutes": (
                    round(avg_duration_minutes, 2) if avg_duration_minutes else 0
                ),
            }
        ...
        if "last_query_contains" in filters:
            stmt = stmt.where(
                and_(
                    ...
                    QuerySessionModel.last_query.like(
