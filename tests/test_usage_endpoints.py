...
def mock_usage_stats():
    ...
    return UsageStats(
        ...
        media_files_count=1,
        queries_count=5,
        daily_limits={"content_limit": 3, "query_limit": 10, "total_content_used": 3},
    )
...
class TestUsageEndpoints:
    ...
    def test_get_usage_stats_success(
        self, client, free_user, mock_usage_stats, mock_db
    ):
        ...
        # Mock usage service
        with patch.object(
            UsageService, "get_usage_stats", return_value=mock_usage_stats
        ):
            # Make request
            response = client.get(
                "/api/v1/usage/", headers={"Authorization": "Bearer fake_token"}
            )
    ...
    def test_get_usage_stats_internal_error(self, client, free_user, mock_db):
        ...
        with patch.object(
            UsageService, "get_usage_stats", side_effect=Exception("Database error")
        ):
            # Make request
            response = client.get(
                "/api/v1/usage/", headers={"Authorization": "Bearer fake_token"}
    ...
    def test_get_subscription_info_success(
        self, client, free_user, mock_subscription_info
    ):
        ...
        with patch.object(
            UsageService, "check_subscription_validity", return_value=free_user
        ):
            with patch.object(
                UsageService,
    ...
    def test_get_subscription_info_premium_user(self, client, premium_user):
        ...
        premium_subscription_info = SubscriptionInfo(
            ...
            daily_limits={"content_limit": 10, "query_limit": 50},
        ...
        with patch.object(
            UsageService, "check_subscription_validity", return_value=premium_user
        ):
            with patch.object(
                UsageService,
                "get_subscription_info",
                return_value=premium_subscription_info,
            ):
                # Make request
                response = client.get(
                    "/api/v1/usage/subscription",
    ...
    def test_get_subscription_info_internal_error(self, client, free_user):
        ...
        with patch.object(
            ...
            data = response.json()
            assert (
                data["detail"]["error"] == "Failed to retrieve subscription information"
            )
            assert "internal error occurred" in data["detail"]["message"]
...
class TestUsageMiddleware:
    ...
    async def test_validate_content_limits_within_limit(self, mock_db, free_user):
        ...
        with patch.object(
            ...
            with patch.object(
                UsageService, "check_daily_content_limit", return_value=True
            ):
    ...
    async def test_validate_query_limits_within_limit(self, mock_db, free_user):
        ...
        with patch.object(
            ...
            with patch.object(
                UsageService, "check_daily_query_limit", return_value=True
            ):
    ...
    async def test_validate_query_limits_exceeded(self, mock_db, free_user):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                side_effect=HTTPException(
                    status_code=429, detail="Query limit exceeded"
                ),
            ):
                with pytest.raises(HTTPException) as exc_info:
