...
class MockRepository(BaseRepository[User, UserCreate, UserUpdate]):
    ...
    async def get_multi(self, db: AsyncSession, *, skip: int = 0, limit: int = 100, filters = None) -> List[User]:
        """Get multiple users."""
        try:
...
class TestBaseRepository:
    ...
    async def test_update_success(self, repository, mock_db, sample_user):
        ...
        result = await repository.update(mock_db, db_obj=sample_user, obj_in=update_data)
    ...
    async def test_get_multi_success(self, repository, mock_db):
        ...
        users = [
            User(id=uuid4(), email="user1@example.com", oauth_provider="google", oauth_subject="1"),
            User(id=uuid4(), email="user2@example.com", oauth_provider="google", oauth_subject="2")
        ]
