...
from app.models.schemas import ContentType, SubscriptionTier, User
...
class TestTextNotesEndpoints:
    ...
    def test_create_text_note_success(
        self, client, mock_db, sample_user, valid_text_note_data
    ):
        ...
        with (
            ...
            response = client.post("/api/v1/notes", json=valid_text_note_data)
        ...
        mock_increment.assert_called_once_with(
            db=mock_db, user_id=sample_user.id, content_type="text_notes"
        )

    def test_create_text_note_minimal_data(
        self, client, mock_db, sample_user, minimal_text_note_data
    ):
        ...
        with (
            ...
            response = client.post("/api/v1/notes", json=minimal_text_note_data)
    ...
    def test_create_text_note_unauthenticated(self, client, valid_text_note_data):
    ...
    def test_create_text_note_validation_error_empty_content(
        ...
        invalid_data = {"text_content": "", "timestamp": datetime.utcnow().isoformat()}
        ...
        with (
            patch(
                "app.services.usage.UsageService.get_or_create_daily_usage"
            ) as mock_get_usage,
            patch(
