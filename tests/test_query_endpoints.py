...
from app.models.schemas import QueryRequest, QueryResponse, User, SubscriptionTier
...
class TestQueryEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_query_timeline_success(self, client: AsyncClient, mock_auth_user, mock_db):
        """Test successful query processing."""
        # Mock the query processing service
        with patch('app.api.v1.endpoints.query.query_processing_service') as mock_service:
            # Mock response
            ...
            with patch('app.api.v1.endpoints.query.validate_query_usage', return_value=True):
                # Make request
    ...
    @pytest.mark.asyncio
    async def test_query_timeline_with_session(self, client: AsyncClient, mock_auth_user, mock_db):
        ...
        with patch('app.api.v1.endpoints.query.query_processing_service') as mock_service:
            ...
            with patch('app.api.v1.endpoints.query.validate_query_usage', return_value=True):
                ...
                response = await client.post(
                    "/api/v1/query/",
                    json={"query": "What did I do yesterday?", "session_id": session_id},
                    headers={"Authorization": f"Bearer {mock_auth_user.token}"}
                )
    ...
    @pytest.mark.asyncio
    async def test_query_timeline_usage_limit_exceeded(self, client: AsyncClient, mock_auth_user, mock_db):
    ...
    @pytest.mark.asyncio
    async def test_query_timeline_service_error(self, client: AsyncClient, mock_auth_user, mock_db):
