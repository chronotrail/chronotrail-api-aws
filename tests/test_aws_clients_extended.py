...
class TestAWSClientsExtended(unittest.TestCase):
    ...
    def test_all_client_creation(self):
        ...
        self.assertEqual(
            rekognition_client.meta.service_model.service_name, "rekognition"
        )
        ...
        self.assertEqual(
            transcribe_client.meta.service_model.service_name, "transcribe"
        )
        ...
        self.assertEqual(
            bedrock_client.meta.service_model.service_name, "bedrock-runtime"
        )
        ...
        self.assertEqual(
            bedrock_embeddings_client.meta.service_model.service_name, "bedrock-runtime"
        )
    ...
    def test_file_size_validation(self):
        ...
        self.assertTrue(
            validate_file_size(5 * 1024 * 1024, 5 * 1024 * 1024)
        )  # 5MB = 5MB

        # Test invalid sizes
        self.assertFalse(
            validate_file_size(10 * 1024 * 1024, 5 * 1024 * 1024)
        )  # 10MB > 5MB
...
class TestAsyncAWSUtils:
    ...
    async def test_upload_file_to_s3(
        self, mock_get_async_s3_client, mock_upload_file, mock_s3_client
    ):
    ...
    async def test_download_file_from_s3(
        self, mock_get_async_s3_client, mock_s3_client
    ):
        ...
        client.download_fileobj.assert_called_once_with(
            "test-bucket", "test-key", unittest.mock.ANY
        )
    ...
    async def test_validate_file(self, mock_upload_file):
        ...
        is_valid, error = await validate_file(
            mock_upload_file, ALLOWED_IMAGE_TYPES, 5, "free"
        )
