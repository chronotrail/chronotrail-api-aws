...
from app.models.schemas import (
    ...
    LocationVisitCreate, TextNoteCreate, QueryRequest, QueryResponse,
    MediaReference, ProcessedPhoto, ProcessedVoice
)
...
def mock_all_services(mock_auth_user):
    ...
    with patch("app.services.usage.UsageService.check_daily_content_limit", return_value=None), \
         patch("app.services.usage.UsageService.increment_content_usage", return_value=None), \
         patch("app.services.usage.UsageService.increment_query_usage", return_value=None), \
         patch("app.services.usage.UsageService.validate_query_date_range", return_value=True), \
         patch("app.middleware.usage_validation.UsageLimitMiddleware.validate_content_limits", side_effect=mock_validate_content_limits), \
         patch("app.middleware.usage_validation.UsageLimitMiddleware.validate_query_limits", side_effect=mock_validate_query_limits):
        yield
...
class TestCompleteUserJourneys:
    ...
    async def test_complete_timeline_creation_and_query_journey(
        ...
        with patch("app.repositories.text_notes.text_note_repository.create", return_value=db_note), \
             patch("app.aws.embedding.embedding_service.process_and_store_text", return_value="doc_note_123"), \
        ...
        with patch("app.aws.storage.file_storage_service.upload_file", side_effect=mock_upload_file), \
             patch("app.repositories.media_files.media_file_repository.create", return_value=db_photo), \
             patch("app.aws.image_processing.image_processing_service.process_image", return_value=processed_photo), \
             patch("app.aws.embedding.embedding_service.process_and_store_text", return_value="doc_photo_123"), \
             patch("app.services.usage.UsageService.increment_content_usage"):
        ...
        with patch("app.aws.storage.file_storage_service.upload_file", side_effect=mock_upload_voice_file), \
             patch("app.repositories.media_files.media_file_repository.create", return_value=db_voice), \
             patch("app.aws.transcription.transcription_service.process_voice", return_value=processed_voice), \
             patch("app.aws.embedding.embedding_service.process_and_store_text", return_value="doc_voice_123"), \
             patch("app.services.usage.UsageService.increment_content_usage"):
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=query_response_data), \
        ...
        with patch("app.repositories.media_files.media_file_repository.get_by_user", return_value=db_photo), \
        ...
        with patch("app.repositories.media_files.media_file_repository.get_by_user", return_value=db_voice), \
             patch("app.aws.storage.file_storage_service.download_file", return_value=fake_audio_data):
            
            voice_retrieval_response = await client.get(
                f"/api/v1/media/{voice_id}",
                headers=auth_header
            )
    ...
    async def test_multi_day_timeline_with_location_search(
        ...
        with patch("app.repositories.location_visits.location_visit_repository.get_by_user_with_date_range", return_value=mock_visits), \
             patch("app.repositories.location_visits.location_visit_repository.count_by_user", return_value=3):
            ...
            visit_times = [visit["visit_time"] for visit in locations_data_response["visits"]]
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=coffee_query_response):
            query_response = await client.post(
                "/api/v1/query/",
                json={
...
class TestConversationalMediaRetrievalFlows:
    ...
    async def test_conversational_photo_discovery_and_retrieval(
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=initial_query_response):
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=followup_query_response):
        ...
        with patch("app.repositories.media_files.media_file_repository.get_by_user", return_value=db_photo):
            info_response = await client.get(
                f"/api/v1/media/{photo_ids[0]}/info",
                headers=auth_header
            )
            ...
            assert info_data["location"]["address"] == "Golden Gate Park, San Francisco, CA"
    ...
    async def test_voice_note_conversation_flow(
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=meeting_query_response):
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=action_items_response):
...
class TestLocationTaggingAndSearchFunctionality:
    ...
    async def test_location_tagging_and_name_based_search(
        ...
        locations_with_tags = [
            ...
            },
            {
                ...
                "latitude": 37.7853,
                "address": "Blue Bottle Coffee, San Francisco, CA",
                "names": ["Blue Bottle Coffee", "Coffee Shop", "Work Spot", "Meeting Place"],
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=work_spot_query):
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=food_query):
        ...
        updated_visit = DBLocationVisit(
            ...
            names=["Golden Gate Park", "Park", "Nature", "Running Spot", "Photography Spot"],
            visit_time=datetime.utcnow() - timedelta(hours=2),
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=photography_query):
            photo_response = await client.post(
                "/api/v1/query/",
                json={
    ...
    async def test_location_based_content_aggregation(
        ...
        with patch("app.services.query.query_processing_service.process_query", return_value=location_aggregation_query):
