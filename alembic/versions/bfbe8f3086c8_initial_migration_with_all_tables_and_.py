...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "oauth_provider", "oauth_subject", name="uq_users_oauth_provider_subject"
        ),
    )
    op.create_index("idx_users_email", "users", ["email"], unique=False)
    op.create_index(
        "idx_users_oauth", "users", ["oauth_provider", "oauth_subject"], unique=False
    )
    op.create_index(
    ...
    op.create_index(
        "idx_location_visits_user_time",
        "location_visits",
        ["user_id", "visit_time"],
        unique=False,
    )
    op.create_index(
        "idx_location_visits_coordinates",
        "location_visits",
        ["longitude", "latitude"],
        unique=False,
    )
    ...
    op.execute(
        "CREATE INDEX idx_location_visits_names ON location_visits USING GIN(names)"
    )
    ...
    op.create_index(
        "idx_text_notes_user_time", "text_notes", ["user_id", "timestamp"], unique=False
    )
    # Create GIN index for array field
    ...
    op.create_table(
        ...
        sa.Column("user_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column(
            "session_context", postgresql.JSON(astext_type=sa.Text()), nullable=True
        ),
    ...
    op.create_index(
        "idx_query_sessions_user_expires",
        "query_sessions",
        ["user_id", "expires_at"],
        unique=False,
    )
